...
public class RegisterManager : MonoBehaviour
{
    ...
    public void Start()
    {
        checkTime = TimeSpan.FromSeconds(CGameDataManager.Instance.playerData.time);

        TimeSpan test =
            DateTime.Now.Subtract(
                DateTime.FromBinary(Convert.ToInt64(CGameDataManager.Instance.playerData.timeRegister)));
        ...
        if (checkTime.TotalSeconds <= 0)
        {
            ...
            CGameDataManager.Instance.playerData.ResetTime();
        }
    ...
    private void AddTime(float i)
    {
        ...
        CGameDataManager.Instance.playerData.SetTimeRegister((long) checkTime.TotalSeconds);
        UpdateTimeRemain();
    }
